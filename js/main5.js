(function() {
    ...
    window.CanvasSlideshow = function( options ) {
      ...
      /*  PIXI VARIABLES
       ---------------------------    
      document -> renderer -> canvas(view) -> stage -> filters (displacementFilter(displ.Sprite)) 
                                                    -> addChild(slidesContainer) --> addChild(image (Pixi.Sprite))
      ...
      */
      ...
      this.initPixi = function() {
        ...
        document.querySelector('#anim-slideshow-holder').appendChild( renderer.view );
        ...
        // Probably tile-ing out the displacement filter image
        displacementSprite.texture.baseTexture.wrapMode = PIXI.WRAP_MODES.REPEAT;


        // Set the filter to stage and set some default values for the animation
        // PIXI.Container() property, receives array
      ...
      this.loadPixiSprites = function( sprites ) {
        ...
        // these are strings -> links to the images (sprites) and text from img sibling element
        var rSprites = options.sprites;
        var rTexts   = options.texts;

        for ( var i = 0; i < rSprites.length; i++ ) {
          
          // first get the image texture and add it to Pixi.Sprite
          ...
          // this is where the image is positioned and scaled
          if ( options.centerSprites === true ) {
            image.anchor.set(0.5);
            image.x = renderer.width / 2;
            image.y = renderer.height / 2;            
          }
          image.transform.scale.x = 1;
          image.transform.scale.y = 1;
